...
impl SqliteDriver {
    ...
    ///
    /// rusqlite is fully synchronous: prepare/step loops block the calling
    /// thread for the duration of the query. Running them inline in an
    /// `async fn` would pin a Tokio worker thread (and hold the connection's
    /// std mutex) for the whole query, starving other async commands. This
    /// helper moves a clone of the `Arc<Mutex<Connection>>` into
    /// `spawn_blocking` so the work leaves the async executor.
    async fn run_blocking<T, F>(&self, f: F) -> Result<T, DbError>
    where
        T: Send + 'static,
        ...
        self.run_blocking(move |conn| {
            ...
            let column_iter = stmt
                .query_map([], |row| {
                    ...
                    let is_auto_increment = (is_primary_key > 0
            ...
            for col in column_iter {
                columns.push(
                    col.map_err(|e| DbError::QueryError(format!("Failed to read column: {}", e)))?,
                );
            }
            ...
            for index_result in index_list_iter {
                ...
                let index_columns = index_columns.map_err(|e| {
                    DbError::QueryError(format!("Failed to read index columns: {}", e))
                })?;
    ...
    async fn get_foreign_keys(&self, schema: &str) -> Result<Vec<ForeignKeyInfo>, DbError> {
        ...
        self.run_blocking(move |conn| {
            ...
            let mut stmt = conn.prepare(query).map_err(|e| {
                DbError::QueryError(format!("Failed to prepare foreign key query: {}", e))
            })?;
            ...
            for row in fk_iter {
                let (tbl, id, _seq, ref_table, from_col, to_col, on_update, on_delete) =
                    row.map_err(|e| {
                        DbError::QueryError(format!("Failed to read foreign key data: {}", e))
                    })?;
