...
- **Redis Key Browser UI**: New `<RedisKeyBrowser>` + route `/_connected/redis-keys` (titlebar View menu item shown only when the active connection's driver is `Redis`). Two-pane resizable layout: cursor-based `SCAN <cursor> MATCH <pattern> COUNT 200` key list (accumulating, de-duped, "Scan more" until cursor `0`, pattern filter, DBSIZE badge) and a value viewer that runs `TYPE`/`TTL` plus the type-appropriate command (`GET`/`HGETALL`/`LRANGE`/`SMEMBERS`/`ZRANGE … WITHSCORES`) and renders string (JSON pretty-print), hash, list, set, zset. Copyable key name, TTL label, per-key Delete (`DEL`) behind a destructive `AlertDialog`. Drives everything through the Redis driver's raw-command `execute_query`; non-Redis connections get a friendly empty state.
...
- **SQL Server Driver Support** (2025-11-22):
  ...
  - Connection to SQL Server 2012 and later versions
  - Full metadata query support (databases, schemas, tables, columns, indexes, foreign keys)
  - Query execution with proper type handling (strings, integers, floats, booleans, UUIDs, datetimes)
  - Schema browser integration for SQL Server databases
  - Table inspector with column and index information
  - Foreign key relationship queries for ER diagrams
  - Added SQL Server to connection form driver dropdown
  - Default port 1433 pre-configured
